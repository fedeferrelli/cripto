      ...
      {operations ? (
        <section className="pb-20">
          <section className="flex flex-col pt-4">
            ...
            <div className="w-full mx-auto text-center text-lg font-light mt-1">
              precio USD{" "}
              {operations.current_price.toFixed(2).toLocaleString("DE-de")}
            </div>
          </section>

          <div className="text-gray-800 font-semibold text-center mt-8  text-2xl">
            <span className="">{operations.operations.length}</span>{" "}
            operaciones:{" "}
          </div>
          ...
          {operations.operations.map((e, index) => (
            <div
              key={index}
              className="w-11/12 bg-gray-200 flex flex-col mt-4 m-auto rounded-md p-3 border border-gray-300/50 shadow-md"
            >
              ...
              <div className="mt-3 text-center text-gray-800 flex flex-row">
                ...
                <div className="w-1/3 text-right flex justify-center items-center">
                  <div className="m-auto text-xl font-semibold w-20 h-20 rounded-full bg-gray-300 shadow-lg flex">
                    <span
                      className={
                        operations.current_price / e.price - 1 < 0
                          ? "text-red-500 m-auto"
                          : "text-green-700 m-auto "
                      }
                    >
                      {((operations.current_price / e.price - 1) * 100).toFixed(
                        2
                      )}
                      %
                    </span>
